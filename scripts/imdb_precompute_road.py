...
VAL_SPLIT_FILE = '/home/shanus/workspace/3D_detection/visualDet3D/visualDet3D/data/kitti/chen_split/train.txt'


def process_synthia_train_val_file(cfg):
    ...
    with open(val_file) as f:
        val_files = json.load(f, object_pairs_hook=collections.OrderedDict)
        for setting in val_files:
            for sample in val_files[setting]:
                val_lines.append(sample)
...
def read_one_split(cfg, index_names, data_root_dir, output_dict,
                   data_split='training', time_display_inter=100):
    ...
    for i, index_name in enumerate(index_names):
        ...
        if cfg.dataset == 'synthia':
            data_frame = SynthiaData(data_root_dir, index_name, output_dict)
        else:
            data_frame = KittiData(data_root_dir, index_name, output_dict)
        calib, image, label, velo, road_map = data_frame.read_data()
        ...
        if data_split == 'training':
            data_frame.label = [obj for obj in label.data
                                if obj.type in cfg.obj_types and
                                obj.occluded < max_occlusion and obj.z > min_z]

            if anchor_prior:
                for j in range(len(cfg.obj_types)):
                    total_objects[j] += len([obj for obj in data_frame.label
        ...
        if data_split == 'training' and anchor_prior:
            ...
            if len(data_frame.label) > 0:
                anchors, _ = anchor_manager(image[np.newaxis].transpose([0, 3, 1, 2]),
                                            torch.tensor(P2).reshape([-1, 3, 4]), road_maps=road_map)

                for j in range(len(cfg.obj_types)):
                    bbox2d = torch.tensor([[obj.bbox_l, obj.bbox_t, obj.bbox_r, obj.bbox_b]
                                           for obj in label if obj.type == cfg.obj_types[j]]).cuda()
                    if len(bbox2d) < 1:
                        continue
                    bbox3d = torch.tensor([[obj.x, obj.y, obj.z, np.sin(2 * obj.alpha), np.cos(2 * obj.alpha)]
                                           for obj in label if obj.type == cfg.obj_types[j]]).cuda()
...
def main(config: str = "config/config.py"):
    ...
    if cfg.dataset == 'synthia':
        train_names, val_names = process_synthia_train_val_file(cfg)
    else:
        train_names, val_names = process_train_val_file(cfg)
    read_one_split(cfg, train_names, data_root_dir, output_dict,
                   'training', time_display_inter)
    output_dict = {
                "calib": True,
                "image": False,
                "label": True,
    ...
    read_one_split(cfg, val_names, data_root_dir, output_dict,
                   'validation', time_display_inter)
