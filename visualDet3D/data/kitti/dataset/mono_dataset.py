...
class KittiMonoDataset(torch.utils.data.Dataset):
    ...
    def collate_fn(batch):
        ...
        return (torch.from_numpy(rgb_images).float(),
                ...
                torch.tensor(calib).float(),
                label, bbox2ds, bbox3ds)
...
class KittiMonoTestDataset(KittiMonoDataset):
    ...
    def __getitem__(self, index):
        ...
        calib = kitti_data.calib
        calib.image_shape = image.shape
        transformed_image, transformed_P2, transformed_road_map = self.transform(
            image, p2=deepcopy(calib.P2), road_map=road_map)
    ...
    def collate_fn(batch):
        ...
        road_map = [item['road_map'] for item in batch]
        return (torch.from_numpy(rgb_images).float(),
                torch.tensor(road_map).float(), calib)
