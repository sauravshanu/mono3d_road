...
class SynthiaMonoDataset(torch.utils.data.Dataset):

    def __init__(self, cfg, split='training'):
        super(SynthiaMonoDataset, self).__init__()
    ...
    def __getitem__(self, index):
        ...
        if index >= len(self.imdb):
            ...
            calib, _, image, _, _, road_map = kitti_data.read_data()
            ...
            _, image, _, _, road_map = kitti_data.read_data()
        ...
        for obj in kitti_data.label:
            if obj.type in self.obj_types:
                label.append(obj)
        transformed_image, transformed_P2, transformed_label, transformed_road_map = self.transform(
                image, p2=deepcopy(calib.P2), labels=deepcopy(label), road_map=road_map)
        bbox3d_state = np.zeros([len(transformed_label), 7]) #[camera_x, camera_y, z, w, h, l, alpha]
        ...
        output_dict = {'calib': transformed_P2,
                       ...
                       'road_map': transformed_road_map,
                       # 'road_map': transformed_image[0],
    ...
    def collate_fn(batch):
        ...
        road_map = [item['road_map'] for item in batch]
        bbox2ds = [item['bbox2d'] for item in batch]
        bbox3ds = [item['bbox3d'] for item in batch]
        return (torch.from_numpy(rgb_images).float(),
                torch.tensor(road_map).float(),
                # road_map,
                torch.tensor(calib).float(), label, bbox2ds, bbox3ds)

@DATASET_DICT.register_module
class SynthiaNuscMonoDataset(SynthiaMonoDataset):
    def __len__(self):
        return len(self.imdb)

@DATASET_DICT.register_module
class SynthiaMonoTestDataset(SynthiaMonoDataset):
    def __init__(self, cfg, split='test'):
        preprocessed_path   = cfg.path.preprocessed_path
        obj_types           = cfg.obj_types
        super(SynthiaMonoTestDataset, self).__init__(cfg, 'test')
        is_train = (split == 'training')
