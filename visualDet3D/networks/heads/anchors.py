...
class InvalidAnchor(Exception):
    '''
    Exception class that can say when the anchor is not
    implemented.
    '''
    pass
...
class Anchors(nn.Module):
    ...
    def __init__(self, preprocessed_path: str,
                 pyramid_levels:List[int], strides:List[float], sizes:List[float], ratios:List[float],
                 scales:List[float], use_z:bool=True, location:str='', readConfigFile:int=1, obj_types:List[str]=[],
                 filter_anchors:bool=True, filter_y_threshold_min_max:Optional[Tuple[float, float]]=(-0.5, 1.8),
                 filter_x_threshold:Optional[float]=40.0, anchor_prior_channel=6):
        ...
        if self.readConfigFile:
            self.anchors_mean_original = np.zeros([len(obj_types), len(self.scales) * len(self.pyramid_levels),
                                                   len(self.ratios), anchor_prior_channel])
            self.anchors_std_original  = np.zeros([len(obj_types), len(self.scales) * len(self.pyramid_levels),
                                                   len(self.ratios), anchor_prior_channel])
    ...
    def forward(self, image:torch.Tensor, calibs:List[np.ndarray]=[], is_filtering=False,
        ...
        if self.shape is None or not (shape == self.shape):
            ...
            if self.readConfigFile:
                ...
                self.anchor_mean_std = torch.stack([self.anchor_means,
                                                    self.anchor_stds], dim=-1).permute(1, 0, 2, 3) #[N, types, 6, 2]
        ...
        if calibs is not None and len(calibs) > 0:
            ...
            if self.readConfigFile and is_filtering:
                ...
                # anchors_z = self.get_anchors_z_simple(road_maps) # [B, types, N]
                if self.location == 'single':
                    anchors_z = self.get_anchors_z_simple(road_maps) # [B, types, N]
                elif self.location == 'three':
                    anchors_z = self.get_anchors_z_from_3_sides(road_maps) # [B, types, N]
                else:
                    raise InvalidAnchor

                world_x3d = (self.anchors_image_x_center * anchors_z -
                             anchors_z.new(cx) * anchors_z) / anchors_z.new(fy) #[B, types, N]
                world_y3d = (self.anchors_image_y_center * anchors_z -
                ...
                self.anchor_mean_std[:, :, :, 0, 0] = anchors_z
