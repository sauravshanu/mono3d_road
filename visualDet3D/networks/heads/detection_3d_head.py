...
from visualDet3D.networks.heads.anchors_z_samplers import RoadBasedAnchors
...
def ravel_multi_index(bottom_points, road_maps):
    road_maps_shape = torch.tensor(road_maps.shape).cuda()
    road_maps_shape[-1] = 1
    bottom_points = bottom_points.permute(1, 0).int()
    return (bottom_points * road_maps_shape).sum(axis=1)
...
class AnchorBasedDetection3DHead(nn.Module):
    def __init__(self, num_features_in:int=1024,
        ...
        self.anchors = RoadBasedAnchors(preprocessed_path=preprocessed_path, readConfigFile=read_precompute_anchor, **anchors_cfg)
    ...
    def get_anchor(self, img_batch, road_maps, P2):
        ...
        anchors, useful_mask, anchor_mean_std = self.anchors(img_batch,
                                                             P2, is_filtering=is_filtering, road_maps=road_maps)
        return_dict=dict(
    ...
    def get_bboxes(self, cls_scores, reg_preds, anchors, P2s, img_batch=None):
        ...
        anchor_mean_std_3d = anchors['anchor_mean_std_3d'][0] #[N, K, 2]
    ...
    def loss(self, cls_scores, reg_preds, anchors, annotations, P2s, road_maps):
        ...
        for j in range(batch_size):
            ...
            anchor_mean_std_3d_j = anchor_mean_std_3d[j][useful_mask]
