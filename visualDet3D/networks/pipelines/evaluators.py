...
from visualDet3D.evaluator.kitti.evaluate import evaluate, evaluate_synthia
...
@PIPELINE_DICT.register_module
@torch.no_grad()
def evaluate_synthia_obj(cfg:EasyDict,
                       model:nn.Module,
                       dataset_val:Sized,
                       writer:SummaryWriter,
                       epoch_num:int,
    ...
    if "is_running_test_set" in cfg and cfg["is_running_test_set"]:
        print("Finish evaluation.")
        return
    result_texts = evaluate_synthia(
        label_path=os.path.join(cfg.path.data_path),
        result_path=result_path,
