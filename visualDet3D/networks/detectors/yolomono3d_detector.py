...
from visualDet3D.networks.detectors.yolomono3d_core import YoloMono3DCore, YoloRoadMono3DCore
...
class Yolo3D(nn.Module):
    ...
    def get_bboxes(self, cls_preds, reg_preds, anchors, P2, img_batch):
        scores, bboxes, cls_indexes = self.bbox_head.get_bboxes(cls_preds, reg_preds, anchors, P2, img_batch)
        return bboxes

    def training_forward(self, img_batch, road_maps, annotations, P2):
        """
        Args:
        ...
        cls_preds, reg_preds = self.bbox_head(dict(features=features, P2=P2, image=img_batch, road_map=road_maps))

        anchors = self.bbox_head.get_anchor(img_batch, road_maps, P2)

        cls_loss, reg_loss, loss_dict = self.bbox_head.loss(cls_preds, reg_preds, anchors, annotations, P2, road_maps)

        return cls_loss, reg_loss, loss_dict, cls_preds, reg_preds, anchors

    def test_forward(self, img_batch, road_maps, P2):
        """
        Args:
            img_batch: [B, C, H, W] tensor
            calib: visualDet3D.kitti.data.kitti.KittiCalib or anything with obj.P2
    ...
    def forward(self, inputs):
        ...
        if isinstance(inputs, list) and len(inputs) == 4:
            img_batch, road_maps, annotations, calib = inputs
            return self.training_forward(img_batch, road_maps, annotations, calib)
        else:
            img_batch, road_maps, calib = inputs
            return self.test_forward(img_batch, road_maps, calib)


class WithoutGroundAwareHead(AnchorBasedDetection3DHead):
    def init_layers(self, num_features_in,
                    num_anchors:int,
                    num_cls_output:int,
                    num_reg_output:int,
...
class RoadYolo3D(Yolo3D):
    ...
    def build_core(self, network_cfg):
        self.core = YoloRoadMono3DCore(network_cfg.backbone)
    ...
    def training_forward(self, img_batch, road_maps, annotations, P2):
        ...
        features  = self.core(dict(image=img_batch, road_maps=road_maps, P2=P2))
        cls_preds, reg_preds = self.bbox_head(dict(
            features=features, P2=P2, image=img_batch, road_map=road_maps))
...
@DETECTOR_DICT.register_module
class WithoutGroundAwareYolo3D(Yolo3D):
    """Some Information about GroundAwareYolo3D"""

    def build_head(self, network_cfg):
        self.bbox_head = WithoutGroundAwareHead(
            **(network_cfg.head)
        )
