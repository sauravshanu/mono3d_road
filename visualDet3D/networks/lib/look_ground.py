...
class LookGround(nn.Module):
    ...
    def forward(self, inputs):
        ...
        road_maps = inputs['road_map'].cpu().numpy()
        road_maps = torch.stack([torch.Tensor(np.array(Image.fromarray(road_map).resize((w, h), Image.NEAREST)))
                                 for road_map in road_maps], axis=0).cuda()
        disparity = 1 / road_maps
