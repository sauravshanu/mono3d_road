...
class LossLogger():
    ...
    def log_image(self, image, reg_preds, road_maps, annotations, P2, step):
        from visualDet3D.utils.visualize_utils import get_bev_pred_gt

        if step % 100:
            return
        idx = 0
        img = vis.np3d(image[idx].cpu().numpy(),
                       out_format={'type': 'np', 'mode': 'RGB', 'channels': 'CHW', 'dtype': 'uint8'})
        reg_preds = reg_preds.unsqueeze(0)
        bev_image = vis.np3d(get_bev_pred_gt(reg_preds[idx].detach().cpu(),
                                             torch.Tensor(annotations[idx]), P2[idx].cpu()),
                             out_format={'type': 'np', 'mode': 'RGB', 'channels': 'CHW', 'dtype': 'uint8'})
        road_map = vis.np2d(road_maps[idx].cpu().numpy(),
                            out_format={'type': 'np', 'mode': 'RGB', 'channels': 'CHW', 'dtype': 'uint8'})
        self.recorder.add_image('image', img, step)
        # self.recorder.add_image('gt_bev', gt_bev, step)
        self.recorder.add_image('bev', bev_image, step)
        self.recorder.add_image('road_map', road_map, step)
